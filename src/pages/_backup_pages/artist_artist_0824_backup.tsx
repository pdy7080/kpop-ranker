...
// 백엔드 응답을 프론트엔드 형식으로 변환 (v11 API 대응)
function transformResponseToArtistData(response: any): ArtistData {
  ...
  const artistNormalized = response.artist || '';

  // v11 API 응답 처리: response.tracks 배열
  if (response.tracks && Array.isArray(response.tracks)) {
    response.tracks.forEach((track: any) => {
      const trackName = track.track;
      const currentRanks = track.current_ranks || {};
      ...
      // current_ranks에서 각 차트 데이터 처리
      Object.entries(currentRanks).forEach(([chartName, rank]) => {
        const chartKey = chartName.toLowerCase();
        if (chartKey in chartData && rank && typeof rank === 'number') {
          chartData[chartKey] = {
            rank: rank,
            views_or_streams: track.youtube_views || null,
            last_updated: new Date().toISOString(),
            in_chart: true,
            rank_class: rank <= 10 ? 'top-10' : rank <= 50 ? 'top-50' : 'in-chart'
          };
          
          // 차트 존재 카운트
          chartPresence[chartKey] = (chartPresence[chartKey] || 0) + 1;
      ...
      let score = 0;
      let chartCount = 0;
      Object.values(chartData).forEach((chartInfo: any) => {
      ...
      tracks.push({
        name: trackName,
        album_image: track.image_url || `/api/album-image-smart/${encodeURIComponent(artistNormalized)}/${encodeURIComponent(trackName)}`,
        charts: chartData,
        streaming_links: {
          spotify: `https://open.spotify.com/search/${encodeURIComponent(artistNormalized + ' ' + trackName)}`,
          apple_music: `https://music.apple.com/search?term=${encodeURIComponent(artistNormalized + ' ' + trackName)}`,
          youtube_music: `https://music.youtube.com/search?q=${encodeURIComponent(artistNormalized + ' ' + trackName)}`,
          melon: `https://www.melon.com/search/song/index.htm?q=${encodeURIComponent(artistNormalized + ' ' + trackName)}`,
          genie: `https://www.genie.co.kr/search/searchMain?query=${encodeURIComponent(artistNormalized + ' ' + trackName)}`,
          bugs: `https://music.bugs.co.kr/search/integrated?q=${encodeURIComponent(artistNormalized + ' ' + trackName)}`,
        },
        trend_score: chartCount > 0 ? Math.round(score / chartCount) : 0
      });
    });
  }
  ...
  const globalPopularityScore = response.stats?.popularity || Math.round(chartDiversity * 100);

  console.log('변환된 데이터:', { tracks: tracks.length, charts: activeCharts });
  ...
  return {
    ...
    tracks,
    stats: {
      total_tracks: response.stats?.total_tracks || tracks.length,
      chart_presence: chartPresence,
      best_ranks: bestRanks,
      best_overall_rank: response.stats?.best_rank || bestOverallRank,
