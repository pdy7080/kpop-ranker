...
export default function ChartOverview({ searchResult, artistName, trackName, onChartClick }: ChartOverviewProps) {
  ...
  const stats = useMemo(() => {
    const chartsWithRank = Object.entries(results)
      ...
      .map(([chartName, data]) => ({
        chartName,
        rank: data.rank!,
        isGlobal: chartConfig[chartName as keyof typeof chartConfig]?.isGlobal || false
      }));
  ...
  const renderChartBox = (chartName: string, data: ChartData, config: any, index: number) => {
    ...
    const isClickable = hasRank || hasYouTubeViews;
      ...
      >
        ...
        {stats.needsAttention > 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="mt-4 p-3 bg-yellow-100 dark:bg-yellow-900/30 rounded-lg border border-yellow-300"
          >
